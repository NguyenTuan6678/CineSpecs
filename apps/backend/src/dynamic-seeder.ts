...
async function run() {
  ...
  const MovieModel = mongoose.model('Movie', MovieSchema) as any;
  const UserModel = mongoose.model('User', UserSchema) as any;
  const ReviewModel = mongoose.model('Review', ReviewSchema) as any;
  ...
  // 1. Seed Users (1 Admin + 5 standard users)
  ...
  // 3. Seed Reviews (30 high-quality reviews across movies)
  ...
  for (const rc of reviewContents) {
    ...
    await ReviewModel.create({
      ...
      movieId: movie._id,
      storyScore: rc.story,
      visual: rc.visual, // wait, our schema is visualScore, actingScore, storyScore
  ...
  // Add 5 reported reviews to test moderation
  ...
  const spamReview = await ReviewModel.create({
    ...
    hasSpoiler: false,
    isReported: true,
    reportCount: 6, // Greater than 5 so it is hidden automatically
  });
